...
p_stat_frm=fopen("stat_frame.dat","r");
  // write to log file
  if (p_stat_frm==0)                      // check if file exist
  {
p_stat_frm=fopen("stat_frame.dat","a");
    if (p_stat_frm==NULL)            // append new statistic at the end
    {
  ...
  else
  {
    fclose (p_stat_frm);
p_stat_frm=fopen("stat_frame.dat","a");
    if (p_stat_frm==NULL)            // File exist,just open for appending
    {
      snprintf(errortext, ET_SIZE, "Error open file %s  \n","stat_frame.dat.dat");
      //error(errortext, 500);
	return 500;
...
//! Currently adding NVB bits on P rate. Maybe additional stats info should be created instead and added in log file
  ...
  if(input->partition_mode ==  PAR_DP_1)
   ...
   else if (input->partition_mode ==  PAR_DP_3)
    ...
    else if(input->of_mode == PAR_OF_RTP)
...
#if !defined(SPEC_CPU)
  // write to log file
p_log=fopen("log.dat","r");
  if (p_log==0)                      // check if file exist
  {
p_log=fopen("log.dat","a");
    if (p_log==NULL)            // append new statistic at the end
    {
      snprintf(errortext, ET_SIZE, "Error open file %s  \n","log.dat");
      //error(errortext, 500);
	return 500;
  ...
  else
  {
    fclose (p_log);
p_log=fopen("log.dat","a");
    if (p_log==NULL)            // File exist,just open for appending
    {
      snprintf(errortext, ET_SIZE, "Error open file %s  \n","log.dat");
      //error(errortext, 500);
	return 500;
